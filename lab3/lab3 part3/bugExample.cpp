 ...
 */
void multArrays(const int arrQuantity[], double arrTotal[][2], const int arrPrice[],  const int size) {
	assert(size > 0);
	const double VAT = 0.21;
	for (int i = 0; i <= size; ++i) {
		arrTotal[i][0] = double(arrQuantity[i] * arrPrice[i] );
	}

	for (int i = 0; i <= size; ++i) {
		arrTotal[i][1] = double (arrQuantity[i] * ( arrPrice[i] + VAT));
	}
 ...
 * @param size The size of the array.
 */
void displayArray(const double arr[][2], const int size) {
	int sum = 0;
